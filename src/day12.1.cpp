...
Pose clockwise(Pose pose, const int degrees) {
    assert(degrees % 90 == 0);
    const auto steps = degrees / 90;
    pose = { pose.getPosition(), clockwise(pose.getHeading(), steps) };
    return pose;
}

Pose counterClockwise(Pose pose, const int degrees)
{
    assert(degrees % 90 == 0);
    const auto steps = degrees / 90;
...
Pose moveHeading(Pose pose, const Vector2D delta)
{
    const auto& poseHead = pose.getHeading();
    pose = { pose.getPosition(),
             Heading { poseHead.getX() + delta.getX(),
                       poseHead.getY() + delta.getY() } };
    return pose;
}
...
class Solution {
    ...
    {
        ++calls;
        Pose pose { Position { 0, 0 }, Heading { 10 , 1 } };
        const auto ret = loop([&](char command, uint16_t arg) {
            switch (command) {
              case 'N':
                pose = moveHeading(pose, Vector2D { 0, arg });
                break;
              case 'S':
                pose = moveHeading(pose, Vector2D { 0, -arg });
                break;
              case 'E':
                pose = moveHeading(pose, Vector2D {arg, 0 });
                break;
              case 'W':
                pose = moveHeading(pose, Vector2D {-arg, 0 });
                break;
